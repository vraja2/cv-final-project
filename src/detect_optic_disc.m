function [confirmed_exudate_map, potential_exudate_map, potential_exudate_cell_array, center_x, center_y, radius] = detect_optic_disc(img, ori_img);
    [h,w,d] = size(img);
 ...
 %   imagesc(img);
  ...
  %  imagesc(CC_img);
   ...
   confirmed_exudate_map = filter_by_boundary(confirmed_exudate_map, vec_img);
   %{
   figure;
   ...
   % load('16_potential_exudates.mat');
 ...
 %  confirmed_exeduate_cell_array = confirmed_exudate_map;
   potential_exudate_map = removed_exudate_map;
 ...
 %  potential_exudate_map = 1;
 %  potential_exudate_cell_array = 1;
   ...
   imagesc(output_img);
   colormap gray;
   confirmed_exudate_map = output_img;
  %}
...
% step 3: 
function output_map = filter_by_boundary(input_map, vec_img)
    %{
    figure;
    imagesc(input_map);
    colormap gray;
%}
    ...
    output_map = get_uint8_img(h,w);
    
    vec_input_map = map_to_vec(input_map, h, w, 1);  
    vec_intensity_img = sum(vec_img,2);
    
    all_connected_regions = bwconncomp(vec_input_map);
    connected_pixel_list = all_connected_regions.PixelIdxList;  
    list_length = size(connected_pixel_list, 2);

    
    
    for i=1:list_length
        p_group = connected_pixel_list{1,i};
        
        
        % check if it's near the boundary of the eye ball
        [p_group_ys, p_group_xs] = ind2sub( [h, w], p_group );
     
        [mean_x, mean_y] = compute_region_center( p_group_xs, p_group_ys);
        vec_boundary_pixels_idx = compute_window1(mean_x, mean_y, h, w, 80);
...
function removed_exudate_map = remove_regions(img_h, img_w, potential_exudate_map, vec_img, vec_circle_pixels)
    ...
    filtered_connected_pixel_length_mask = (connected_pixel_length_list >= 15);
    ...
    for i=1:list_length
        ...
        % check if it is too red, could be veins
        p_group_r = double(vec_img(p_group, 1));
        p_group_g = double(vec_img(p_group, 2));
        p_group_b = double(vec_img(p_group, 3));
        
        p_group_rg_ratio = mean(p_group_r./p_group_g);
    %    p_group_rb_ratio = p_group_r./p_group_b; 
        
        if( p_group_rg_ratio > 1.5)
 %           continue;
        end
...
function vec_boundary_pixels_idx = compute_window1(cx, cy, img_h, img_w, window_width)
...
function [hor_median, vert_median] = compute_50_pixel_median_filter(img_h, img_w, img)
  ...
  %  load('16_left_hor_median.mat');
  %  load('16_left_vert_median.mat')
    
  %{
    load('16410_left_hor_median.mat');
    load('16410_left_vert_median.mat')
  %}
  
  
%    load('4793_left_hor_median.mat');
 %   load('4793_left_vert_median.mat')
...
function confirmed_exudate_map = get_confirmed_exudate(h, w, subtracted_img, vec_img, vec_circle_pixels)
 ...
 %   r_pixels = reshape(r_pixels, [h,w]);
    ...
    [filtered_ys, filtered_xs] = find(filtered_intensity_map == 1);
    
    [m1,n1] = size(filtered_ys);
    %{
    for i1=1:m1
        
        vec_boundary_pixels_idx = compute_window(filtered_xs(1,i1), filtered_ys(1,i1), h, w);
        boundary_pixels = vec_intensity_img(vec_boundary_pixels_idx);
        if( ~all(boundary_pixels))
            continue;
        end
...
%{    
  ...
  %}  
    ...
    eye_ball_left_edge_intensity = eye_ball_center_x_pixels(1, eye_ball_left_edge:eye_ball_left_edge+10, :);
    eye_ball_right_edge_intensity = eye_ball_center_x_pixels(1, eye_ball_right_edge:eye_ball_right_edge+10, :);
 %{
...
%}
    ...
    red_color = double(sum(eye_ball_left_edge_intensity(1,:,1)));
    green_color = double(sum(eye_ball_left_edge_intensity(1,:,2)));
    blue_color = double(sum(eye_ball_left_edge_intensity(1,:,3)));
    ...
    vec_circle_pixels = get_circle_pixel_list(img_h, img_w, eye_ball_center_x, eye_ball_center_y, eye_ball_radius + 100);
